...
lab:
  ...
  description: Learn how to use Azure Load Testing to simulate performance testing
    against live-running web applications with different load scenarios.
  duration: 35 minutes
  level: 500
  islab: true
  primarytopics:
...
1. Give your project the name **eShopOnWeb** and choose **Scrum** on the **Work Item process** dropdown
1. Select **Create**
...
1. Select **Repos > Files**, then **Import**
...
1. In the toolbar, select the **Cloud Shell** icon located directly to the right of the search text box
1. If prompted to select either **Bash** or **PowerShell**, select **Bash**

> **Note**: If this is the first time you are starting Cloud Shell and you are presented with the "You have no storage mounted" message, select the subscription you are using in this lab, and select **Create storage**.

1. From the **Bash** prompt, run the following command to create a resource group (replace `<region>` with the name of the Azure region closest to you such as 'eastus'):
...
1. Select **New pipeline** (or Create Pipeline if this is the first one you create)
1. On the **Where is your code?** pane, select **Azure Repos Git (YAML)** option
...
1. **Copy** the full template pipeline from below:
...
1. Set the cursor on a new line at the end of the YAML definition at the indentation of the previous task level
1. Select **Show Assistant** from the right hand side of the portal
1. In the list of tasks, search for and select the **Azure App Service Deploy** task
1. In the **Azure App Service deploy** pane, specify the following settings and select **Add**:
...
1. Confirm the settings from the Assistant pane by selecting the **Add** button
...
1. Before saving the updates, give it a clear name. On top of the yaml-editor window, rename **azure-pipelines-#.yml** to **m08l14-pipeline.yml**
1. Select **Save**, on the **Save** pane, select **Save** again to commit the change directly into the main branch
1. Navigate to **Pipelines** and select **Pipelines** again. Select **All** to open all pipeline definitions
1. Select the pipeline and confirm to run it by selecting **Run pipeline** and confirm by selecting **Run** once more
...
1. On the Azure web app blade, select **Overview** and select **Browse** to open your site in a new browser tab
...
1. In the search field, enter **Azure Load Testing**
...
1. Provide the necessary details for the resource deployment:
...
1. Select **Review and Create**, to validate your settings
1. Select **Create** to confirm and deploy the Azure Load Testing resource
1. Wait for the deployment to complete successfully
1. Select **Go to Resource** to navigate to the **eShopOnWebLoadTesting** Azure Load Testing resource
...
1. Select the **+ Create** menu option, and select **Create a URL-based test**
1. Complete the following parameters and settings:
...
1. The test will run for 5 minutes

#### Create second load test (Requests per Second)

1. From the top menu, select **Create**, **Create a URL-based test**
1. Complete the following parameters and settings:
...
1. Select either of the test definitions to open a detailed view by **selecting** on one of the tests
1. Select the **TestRun_mm/dd/yy-hh:hh** from the resulting list
1. From the detailed **TestRun** page, identify the actual outcome of the simulation:
...
1. Review the dashboard graph line and chart views below
1. Take time to **compare the results** of both simulated tests and **identify the impact** of more users on the App Service performance
...
1. From the bottom left corner, select **Project Settings**
1. Under the **Pipelines** section, select **Service Connections**
1. Notice the Service Connection with the name of your Azure Subscription
1. **Select the Service Connection**. From the **Overview** tab, navigate to **Details** and select **Manage service connection roles**
1. This redirects you to the Azure Portal, opening the resource group details in the access control (IAM) blade
...
1. In the **Members tab**, select **Select members**, find and select your user account and select **Select**
...
1. On the left pane, select **Tests** to view the list of load tests, then select **your test**
1. Select the **ellipsis (...)** next to the test run, then select **Download input file**
1. The browser downloads a zipped folder that contains the load test input files
1. Use any zip tool to extract the input files. The folder contains:

   - _config.yaml_: the load test YAML configuration file
   - _quick_test.jmx_: the JMeter test script

1. Navigate to the **Azure DevOps Portal** and the **eShopOnWeb** DevOps Project
1. Select **Repos**. In the source code folder structure, notice the **tests** subfolder
1. Notice the ellipsis (...), and select **New > Folder**
1. Specify **jmeter** as folder name, and **placeholder.txt** for the file name
1. Select **Commit** to confirm the creation of the placeholder file and jmeter folder
1. From the **Folder structure**, navigate to the new created **jmeter** subfolder
1. Select the **ellipsis(...)** and select **Upload File(s)**
...
1. Select **Commit** to confirm the file upload into source control
1. Within Repos, browse to the **/tests/jmeter** subfolder
1. Open the Load Testing **config.yaml** file. Select **Edit** to allow editing
1. Replace the **displayName** and **testId** attributes with the value **ado_load_test**
1. Select **Commit** to save the changes
...
1. From the Azure DevOps Portal, navigate to **Pipelines** and select the pipeline created earlier. Select **Edit**
1. In the YAML script, navigate to **line 56** and press ENTER/RETURN to add a new empty line (right before the Deploy Stage)
1. At line 57, select the Tasks Assistant and search for **Azure Load Testing** (make sure you position the cursor at the indentation of the previous task level).
1. Complete the graphical pane with the correct settings:
...
1. Select **Add** to inject the parameters as a YAML snippet
1. Fix indentation if needed (add 2 spaces or tab to position the snippet correctly)
...
1. Below the inserted YAML snippet, add a new empty line
1. Add a snippet for the Publish task to show the results of the Azure Load testing task:
...
1. Fix indentation if needed
1. **Save** the changes
1. Once saved, select **Run** to trigger the pipeline
1. Confirm the branch (main) and select **Run** to start the pipeline run
1. From the pipeline status page, select the **Build** stage to open the verbose logging details
1. Wait for the pipeline to arrive at the **AzureLoadTest** task
1. While the task is running, browse to the **Azure Load Testing** in the Azure Portal and see how the pipeline creates a new RunTest named **adoloadtest1**
1. Navigate back to the Azure DevOps CI/CD Pipeline Run view, where the **AzureLoadTest task** should complete successfully
...
1. Within Repos, browse to the **/tests/jmeter** subfolder
1. Open the Load Testing **config.yaml** file. Select **Edit** to allow editing
1. Replace `failureCriteria: []` if present, otherwise append the following snippet:
...
1. Save the changes by selecting **Commit** and Commit once more
1. Navigate back to **Pipelines** and run the **eShopOnWeb** pipeline again
1. After a few minutes, it will complete with a **failed** status for the **AzureLoadTest** task
1. Open the verbose logging view for the pipeline, and validate the details of the **AzureLoadtest**
...
1. Notice how the last line says **##[error]TestResult: FAILED**. Since we defined a **FailCriteria** having an avg response time of > 300, the task is flagged as failed when the response time exceeds this threshold
...
1. This will remove all associated resources including the App Service, App Service Plan, and Load Testing resource
